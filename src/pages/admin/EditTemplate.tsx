  ...
  const fetchTemplate = async () => {
    try {
      ...
      setFormData({
        ...
        jsonUrlBoy: data.json_url_boy || '',
        jsonUrlGirl: data.json_url_girl || '',
        tags: data.tags ? data.tags.join(', ') : '',
        gender: data.gender || 'unisex',
        priceEur: data.price_eur.toString(),
        isPublished: data.is_published,
      })

      if (data.cover_image_url) {
        setUploadedImageUrl(data.cover_image_url)
      }
    } catch (err) {
  ...
  const removeImage = async () => {
    if (uploadedImageUrl) {
      try {
        ...
        if (fileName && !uploadedImageUrl.includes(formData.coverImageUrl)) {
